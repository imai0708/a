        ...
        <form action="{{ route('posts.update', $post) }}" method="POST" class="rounded pt-3 pb-8 mb-4">
            ...
            <div class="mb-4">
                ...
                <select name="advisor_id"
                    ...
                    @foreach ($occupations as $occupation)
                        <option value="{{ $occupation->id }}" @if ($occupation->id == old('occupation_id', $job_offer->occupation_id)) selected @endif>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="due_date"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="募集期限" value="{{ old('due_date', $post->due_date) }}">
            </div>
            <div class="mb-4">
                ...
                <textarea name="description" rows="10"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="詳細">{{ old('description', $post->description) }}</textarea>
            </div>
            <div class="mb-4">
                <label class="block text-white mb-2" for="description">
                    公開状況
                ...
                @foreach (App\Models\JobOffer::STATUS_LIST as $value => $name)
                    <input type="radio" name="is_published" value="{{ $value }}"
                        @if ($value == old('is_published', $post->is_published)) checked="checked" @endif required>
